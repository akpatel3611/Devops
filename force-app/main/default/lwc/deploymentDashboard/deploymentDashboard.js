...
import saveSelectedComponents
from '@salesforce/apex/MetadataSaveController.saveSelectedComponents';
...
export default class DeploymentDashboard extends LightningElement {
    ...
    @track deploymentLogs = [];
    @track pollingMessage = '';
    ...
    get deploymentProgress() {
        switch (this.deploymentStatus) {
            case 'Waiting For Pull Request Approval':
                return 25;
            case 'Pending':
                return 50;
            case 'Validating':
    ...
    get isWaitingMerge() {
        return this.deploymentStatus === 'Waiting For Pull Request Approval';
    }
        ...
        if(!exists){
            ...
            this.selectedComponents = [
                ...this.selectedComponents,
                {
                    label,
                    value,
                    type: this.selectedMetadataType
                }
            ];
        }
    }
    ...
    handleSave() {
        ...
        .then(() => {
            const filePaths = this.selectedComponents.map(item => 
                this.getFilePath(item.type, item.value)
            );
    ...
    handleDeploy() {
        ...
        .then(result => {
            if (result.success) {
                ...
                this.prNumber = result.pullRequestNumber;
                 this.prUrl = result.pullRequestUrl;
                 this.featureBranchName = result.featureBranch;
                 this.isDeploying = true;
                 this.deploymentStatus = 'Waiting For Pull Request Approval';
                 this.showDeployButton = false;
                this.deploymentLogs = [];
                ...
                // Start polling GitHub and Salesforce status
    ...
    startStatusPolling() {
        ...
        this.pollingIntervalId = setInterval(() => {
            checkPRStatusAndDeploy({
                userStoryId: this.userStoryId,
                prNumber: this.prNumber,
                prUrl: this.prUrl,
                branchName: this.featureBranchName
            })
             .then(result => {
                 this.deploymentStatus = result.status;
                 this.pollingMessage = result.message || '';
                 if (result.logs) {
                    this.deploymentLogs = result.logs.map(log => {
                        return {
                            ...log,
                            formattedTime: log.Timestamp__c ? new Date(log.Timestamp__c).toLocaleTimeString() : ''
                        };
                    });
                }
                ...
                if (result.status === 'Completed' || result.status === 'Failed') {
                    ...
                    if (result.status === 'Completed') {
                        this.dispatchEvent(new ShowToastEvent({
                            ...
                            message: 'Metadata successfully deployed to target organization!',
                            variant: 'success'
                        }));
                    } else {
                        this.dispatchEvent(new ShowToastEvent({
                            title: 'Deployment Failed',
                            message: 'Metadata deployment failed. Review logs for details.',
                            variant: 'error'
                        }));
                    }
                }
